...
1. Apply `developer/migrations/portal/011_add_10dlc_registration.sql`, `012_add_business_legal_address.sql` and `013_add_10dlc_campaign_links.sql` to **`mxhyvvgjtqllohpvrwon`** — the project `PORTAL_SUPABASE_URL` points at, where `businesses` lives. Not `hltmzafywzqajjzjpqva` (the CRM project behind plain `SUPABASE_URL`), which has no `businesses` table and fails with `42P01`. `/portal-admin/api/config-debug` prints the host the portal reads.
